    ...
    return (
        <div className={classes.usersBox}>
            <div className={classes.paginatorWrap}>
                {pages.map(n => {
                    return (props.currentPage - 3 <= n && n <= props.currentPage + 3) ? <span key={n} className={props.currentPage === n && classes.selectedPage} onClick={(e) => {
            ...
            {props.users.map( (item) =>
                <User key={item.id}
                      ...
                      onClick={() => {props.changeFriendStatus(item.id)}}
